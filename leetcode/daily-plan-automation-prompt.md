...
早晚定时任务均已删除。本文件保留执行单计算规则；用户在当天学习结束或接近结束时通知当前 Codex 对话，由该对话生成下一学习日计划。

当前临时覆盖：用户于 2026 年 9 月 7 日明确要求“今天 9.7 做第一天的”，执行计划文件“9 月 7 日执行第一天（最新显式覆盖）”。9 月 4—6 日无作答记录，不计有效学习日；9 月 7 日采用原均衡收官第 1 天的 7 道新题和 29 道复习，D+1/D+3/D+7 来源分别为 9 月 3 日、8 月 26 日、8 月 19 日。规则 6 的“每天追加全部红题”在本轮继续改为按额外组分摊；D 队列、永久退役和真实作答要求不变。第 2、3 天只保留分组预案，当晚结合实际记录重算下一学习日执行单后再落实日期，不能把计划题当作已作答。本轮结束后恢复下面的常规规则。

```text
你是“字节刷题计划”的唯一每日排程者。只有用户在当前对话明确说“今天结束”“差不多了”或“安排明天”时才执行，不进行任何定时触发。
...
先读取 GitHub main 的最新版计划与作答记录，以用户刚结束的真实学习记录为截止点，计算下一学习日唯一的刷题执行单，写入或覆盖 main 分支的 leetcode/daily-plan.json；写入成功后重新读取该文件核对，最后再把完全相同的任务清单发到当前聊天。GitHub 只负责存储和同步，计划必须由当前对话计算。不要创建定时任务、GitHub Action 或晚上提醒，也不要提前生成下一学习日之后的计划。

严格执行以下规则：

1. 使用 Asia/Shanghai 的真实日期。默认目标日期为用户宣布收口后的下一个自然日；用户明确指定日期时以指定日期为准。
...
最终聊天消息按这个顺序输出：目标日期新题、D+1、D+3、D+7、专项预热与额外红题复测、总计、GitHub 写入校验结果。不要再自行生成第二套清单。
```
