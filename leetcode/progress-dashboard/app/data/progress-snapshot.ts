...
export const progressSnapshot: Omit<DashboardPayload, "dailyPlan"> = {
  ...
  "attempts": [
    ...
    },
    {
      ...
      "status": "红",
      "independentWrite": false,
      "errorReason": "首次作答只能判断本题可能使用 DFS，但尚未建立递归函数是对有序数组的当前区间构造平衡 BST 并返回根节点的含义，也未独立想到每次选择区间中点创建当前根节点、用左半区间构造左子树并用右半区间构造右子树的完整构造模型",
      "isReview": false,
      "reviewDate": null,
      "notes": "红｜首次作答尚未建立有序区间递归构造平衡 BST 的模型｜未作答→红；dfs(left right) 返回由 nums[left:right+1] 构造出的树根，选区间中点为根后递归构造 left..middle-1 和 middle+1..right，left > right 时返回 None",
      "sourceRow": 163,
      "isVoid": false,
      "supersedesAttemptId": null,
      "correctionReason": null,
    ...
    },
    {
      ...
      "status": "黄",
      "independentWrite": false,
      "errorReason": "首次作答已独立想到对节点使用 DFS 检查左子节点值小于当前值且右子节点值大于当前值，掌握了 BST 的局部大小方向；但只比较直接孩子无法保证整个左子树和右子树满足所有祖先范围，也未处理空节点与空孩子；并且在递归调用之前已经返回布尔值，导致左右子树的递归代码永远不可达",
      "isReview": false,
      "reviewDate": null,
      "notes": "黄｜首次作答已掌握 BST 左小右大的方向，但尚未建立向下传递全局合法上下界的验证模型｜未作答→黄；dfs(node lower upper) 检查 lower < node.val < upper，左子树继承下界并将上界收紧为 node.val，右子树继承上界并将下界收紧为 node.val",
      "sourceRow": 164,
      "isVoid": false,
      "supersedesAttemptId": null,
      "correctionReason": null,
    ...
    },
    {
      ...
      "status": "红",
      "independentWrite": false,
      "errorReason": "首次作答明确表示没有思路，尚未独立利用前序遍历按根、左、右的顺序确定当前子树根节点，也未利用根节点在中序遍历中的位置切分左右子树范围，未建立递归构造左右子树并返回当前根节点的建树模型",
      "isReview": false,
      "reviewDate": null,
      "notes": "红｜首次作答尚未建立前序取根、中序分割左右区间的递归建树模型｜未作答→红；用哈希表保存值到中序下标的映射，preorder_index 指向下一个根值，dfs(inorder_left inorder_right) 按前序顺序先建左子树再建右子树并返回当前根",
      "sourceRow": 165,
      "isVoid": false,
      "supersedesAttemptId": null,
      "correctionReason": null,
    ...
    },
    {
      ...
      "status": "黄",
      "independentWrite": false,
      "errorReason": "本次 D+7 复习明确表示原地哈希标记的整体思路已经很清楚，但主动跳过完整盲写，没有提交代码并验证非正数替换、绝对值取原数、数字 x 映射到下标 x-1 以及最终返回 n+1 等边界，因此不能按独立通过验收",
      "isReview": true,
      "reviewDate": "2026-08-25",
      "notes": "黄｜D+7 已口述掌握但未进行要求的完整盲写｜黄→黄；理解不等于本次独立通过，保持黄色",
      "sourceRow": 167,
      "isVoid": false,
      "supersedesAttemptId": null,
      "correctionReason": null,
    ...
    },
    {
      ...
      "status": "黄",
      "independentWrite": false,
      "errorReason": "本次 D+7 复习独立恢复了前缀和加哈希表的主方向，但 need 误写为 k-nums[i] 而不是 prefix_sum-k；命中时只给答案加 1，未累加所需前缀和的出现次数；记录当前前缀和时直接覆盖为 1；并且缺少初始状态 {0: 1}",
      "isReview": true,
      "reviewDate": "2026-08-25",
      "notes": "黄｜D+7 主模型正确但四个关键实现点未通过｜黄→黄；先用 prefix_count.get(prefix_sum-k, 0) 累计答案，再把当前前缀和频次加一，初始哈希表必须包含 0:1",
      "sourceRow": 168,
      "isVoid": false,
      "supersedesAttemptId": null,
      "correctionReason": null,
    ...
    },
    {
      ...
      "status": "红",
      "independentWrite": false,
      "errorReason": "今日 D+1 复习再次直接要求讲解，未先独立口述或实现；讲解覆盖前序遍历下一个未使用值确定当前子树根节点、根值在中序遍历中的位置切分左右区间、使用哈希表将根位置查找降为常数时间、build(left、right) 返回当前中序区间构造出的子树根节点，以及必须按照前序的根左顺序先构造左子树再构造右子树，但本次尚未自行写出代码并提交验证",
      "isReview": true,
      "reviewDate": "2026-08-26",
      "notes": "红｜D+1 再次依赖完整讲解且双序列递归建树模型尚未独立恢复｜红→红；前序指针负责依次拿根，中序下标负责划分左右子树范围，build 返回建好的子树根节点且必须先左后右",
      "sourceRow": 198,
      "isVoid": false,
      "supersedesAttemptId": null,
      "correctionReason": null,
    ...
    },
    {
      ...
      "status": "红",
      "independentWrite": false,
      "errorReason": "今日 D+3 复习未尝试独立口述或编写代码而直接请求完整讲解，仍未自行恢复后序 DFS 的双重职责；讲解再次说明对子树负贡献与零取较大值，在当前节点使用 node.val 加左右两边非负贡献更新可拐弯的全局答案，但向父节点只能返回 node.val 加左右较大一侧，且全局答案必须初始化为负无穷以处理全负数树",
      "isReview": true,
      "reviewDate": "2026-09-03",
      "notes": "红｜D+3 仍依赖完整讲解，单边返回与双边更新模型尚未独立恢复｜红→红；dfs 向父节点返回从当前节点出发的一条最大单边路径，全局 ans 在当前节点用 node.val+left+right 更新，负贡献直接舍弃",
      "sourceRow": 215,
      "isVoid": false,
      "supersedesAttemptId": null,
      "correctionReason": null,
    ...
    },
    {
      ...
      "status": "红",
      "independentWrite": false,
      "errorReason": "今日 D+3 复习未独立口述或重写代码而直接进入完整讲解，仍未自行恢复区间递归建树模型；讲解再次说明 build(left、right) 返回当前闭区间构造出的子树根节点，区间为空时返回 None，选择中间元素创建根节点并分别连接左右半区间的递归结果，中点切分同时保证 BST 大小关系与左右高度平衡",
      "isReview": true,
      "reviewDate": "2026-09-03",
      "notes": "红｜D+3 仍依赖完整讲解，区间参数和返回子树根节点的递归模型尚未独立恢复｜红→红；每次选区间中点为根，左半区递归构造左子树，右半区递归构造右子树，最后返回当前根节点",
      "sourceRow": 217,
      "isVoid": false,
      "supersedesAttemptId": null,
      "correctionReason": null,
    ...
    },
    {
      ...
      "status": "红",
      "independentWrite": false,
      "errorReason": "在同日完整讲解后立即重新编写，虽已复现中序值到下标的哈希表、前序全局指针、闭区间递归边界、按根左右顺序连接子树及返回根节点，但该次紧随答案不计独立恢复；代码仍将整数 root 覆盖为 TreeNode 后用于 inorder_index[root]，混淆节点对象与节点值导致查表失败，末尾另有多余的 nacuol 文本，尚未独立通过",
      "isReview": true,
      "reviewDate": "2026-09-03",
      "notes": "红｜讲解后复现不计独立掌握且代码仍未通过｜红→红；需隔开答案后从空白独立写出并通过，middle 应通过保留的 root_value 查询",
      "sourceRow": 219,
      "isVoid": false,
      "supersedesAttemptId": null,
      "correctionReason": null,
    ...
    },
    {
      ...
      "status": "绿",
      "independentWrite": true,
      "errorReason": "无新增错误；用户反馈“167 秒了”，并明确说明“我秒了就是绿”。按用户自述独立通过记录，直接标记绿色；本条依据用户反馈，不声称助手已运行其代码验收。",
      "isReview": true,
      "reviewDate": "2026-09-14",
      "notes": "绿｜用户自述秒了，独立通过｜黄→绿；纠正本次 167 复习被保守误标为黄色的问题，原位修正同一条记录，未新增作答次数。今后用户在对话中说秒了即按其自述独立通过记绿，无需另附代码或运行结果。",
      "sourceRow": 247,
      "isVoid": false,
      "supersedesAttemptId": null,
      "correctionReason": null,
