...
## 第二遍分类复习（2026-09-14 起生效）

- [完整 16 天计划](../2026-09-topic-review-plan.md)，唯一排程数据为 [`../review-plan.json`](../review-plan.json)。9/14–9/29 每天 4–8 题，题量较少的相近类别合并，按 Asia/Shanghai 日期自动选择预先写好的类别，不再混入 D+1/D+3/D+7 队列。
- 97 题进入每日计划；历史退役的 42、73、136 保留卡片供主动查阅。两个 `app/data/review-cards-*.json` 合计覆盖全部 100 题，各有改写题干、例子、思路、步骤、易错点、复杂度及目录中的力扣来源链接。
- `/review` 默认今日类别，开始前预览第一天，结束后提示补练。支持 `?date=2026-09-14&problem=283` 定位日期和题目、题号/题名检索、未练/薄弱题筛选、逐题切换和完整日历。
...
- 对话中用户说某题“秒了”，按其 9/14 明确约定直接记为绿色、独立通过，并注明依据为用户自述；无需另附代码或运行结果，记录后同步提交推送。
- 首页保留 9/11 已完成的执行单；9/14 起由 `categoryPlanForDate` 将固定分类日历转换为首页当天执行单，不依赖定时任务。
